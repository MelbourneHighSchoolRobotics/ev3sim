...
class Image(Colorable):
    ...
    def image_path(self, value):
        ...
        image_path = find_abs(value, allowed_areas=["local", "local/assets/", "package", "package/assets/"])
    ...
    def calculatePoints(self):
        ...
        if isinstance(self.scale, (tuple, list)):
            new_size = [
                int(self.image.get_size()[0] * self.scale[0] * relative_scale[0]),
                int(self.image.get_size()[1] * self.scale[1] * relative_scale[1]),
            ]
        else:
            new_size = [
